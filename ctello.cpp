...
bool Tello::Bind(const int local_client_command_port)
{
    ...
    result = ctello::FindSocketAddr(TELLO_SERVER_IP, TELLO_SERVER_COMMAND_PORT,
...
bool Tello::SendCommand(const std::string& command)
{
    const std::vector<unsigned char> message{std::cbegin(command),
                                             std::cend(command)};
    const auto result =
        ctello::SendTo(m_command_sockfd, m_tello_server_command_addr, message);
...
std::optional<std::string> Tello::ReceiveResponse()
{
    ...
    const auto result = ctello::ReceiveFrom(
...
std::optional<std::string> Tello::GetState()
{
    ...
    const int size{1024};
    std::vector<unsigned char> buffer(size, '\0');
    const auto result = ctello::ReceiveFrom(m_state_sockfd, addr, buffer, size);
    const int bytes{result.first};
    if (bytes < 1)
    {
        return {};
